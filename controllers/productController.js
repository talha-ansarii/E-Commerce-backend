...
exports.createProduct = async (req, res) => {
  const body = req.body; 
  const { success, errors, sanitizedBody } = validateAndSanitize(createProductSchema, body);
  if (!success) {
  ...
  try {
    ...
    const insertQuery = 'INSERT INTO products (name, description, price, stock, category_id) VALUES (?, ?, ?, ?, ?)';
    ...
    res.status(201).json({ message: "Product created!", id: productResults.insertId }); 
  } catch (err) {
    res.status(500).json({ error: err.message }); 
  }
};
...
exports.updateProduct = async (req, res) => {
  const { id } = req.params; 
  const body = req.body; 
  const { success, errors, sanitizedBody } = validateAndSanitize(updateProductSchema, body);
  if (!success) {
    return res.status(400).json({ error: 'Validation failed', ...errors }); 
  }
  ...
  try {
    ...
    if (results.affectedRows === 0) return res.status(404).json({ error: 'Product not found' }); // Handle not found case
